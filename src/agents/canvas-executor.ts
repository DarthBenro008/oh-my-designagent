...
export function createCanvasExecutorAgentWithOverrides(args: {
  ...
  let { override } = args
  const { systemDefaultModel, memorySummary, figmaUseEnabled, figmaUseServerName } = args

  if (override?.disable) {
    override = undefined
  }

  const model = override?.model ?? systemDefaultModel ?? CANVAS_EXECUTOR_DEFAULTS.model
  const temperature = override?.temperature ?? CANVAS_EXECUTOR_DEFAULTS.temperature
  ...
  if (override?.fallback_models !== undefined) {
    ;(config as AgentConfig & { fallback_models?: AgentOverrideConfig["fallback_models"] }).fallback_models =
